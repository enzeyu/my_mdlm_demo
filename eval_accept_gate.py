"""Evaluate utility-aware accept gates for GPT-2 + LoRA-MDLM refinement."""
...
from draft_aware_lora_utils import freeze_module, select_uncertain_blocks
...
def evaluate(config: dict, gate_ckpt: str | None) -> list[dict]:
    ...
    accs: dict[tuple[str, float | str], dict] = {
        ...
        ("draft_aware_lora_refine_with_rule_gate", ""): new_acc(),
    }
...
def write_summary(path: Path, rows: list[dict], best: dict | None) -> None:
    ...
    rule = by_mode.get("draft_aware_lora_refine_with_rule_gate")
    learned = best
    lines = [
    ...
    if learned is None:
        lines.extend(
            [
                ...
                "4. gate 是否优于 rule-based gate？未评估。",
                "5. 当前最佳 threshold 是多少？未评估。",
            ]
        )
    else:
        ...
        beats_rule = bool(rule and (learned["net_correction"], learned["top1"]) > (rule["net_correction"], rule["top1"]))
        lines.extend(
            [
                ...
                f"4. gate 是否优于 rule-based gate？{'是' if beats_rule else '否或证据不足'}。",
                f"5. 当前最佳 threshold 是多少？`{float(learned['gate_threshold']):.2f}`。",
            ]
        )
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run(config_path: str, gate_ckpt: str | None, eval_steps: int | None = None, save_dir: str | None = None) -> list[dict]:
    ...
    save_csv(out_dir / "accept_gate_eval.csv", rows)
    save_json(out_dir / "accept_gate_eval.json", {"benchmark": rows})
    if best is not None:
        save_json(out_dir / "accept_gate_best_config.json", {**best, "gate_ckpt": gate_ckpt})
    write_summary(out_dir / "accept_gate_summary.md", rows, best)
    print(format_table(rows, COLUMNS))
    print(f"saved_accept_gate_eval={out_dir / 'accept_gate_eval.csv'}")
    return rows
